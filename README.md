...
- 每张图右下角有「**存 SVG / 存 PNG**」，点一下就把当前这张图存下来；SVG 是矢量，放大不糊，直接塞文档、推文都行。
...
**`chart.html` 打开一片空白**
页面里的 ECharts 是从 CDN 拉的，断网就没图。要么联网，要么把 `main.cpp` 里那行 `<script src="https://cdn.jsdelivr.net/npm/echarts@5/dist/echarts.min.js">` 换成本地文件。
